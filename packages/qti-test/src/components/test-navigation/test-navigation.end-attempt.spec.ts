 ...
 * then (a failed assessment.xml fetch leaves them enabled forever), so each
 * handler has to tolerate a missing test, item-ref, assessment item or session
 * context and simply do nothing.
 */
describe('TestNavigation event handlers without a loaded test', () => {
  ...
  const connectTest = (nav: TestNavigation, assessmentItem?: unknown): void => {
    ...
    if (assessmentItem !== undefined) {
      Object.assign(test.querySelector('qti-assessment-item-ref')!, { assessmentItem });
    }
    nav.dispatchEvent(
  ...
  const candidateEvents = [
    ...
    'test-show-candidate-correction',
